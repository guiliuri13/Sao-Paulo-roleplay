...
------------------------------------------
vRP._prepare("vRP/getContactsNotify","SELECT display FROM phone_users_contacts WHERE phone_users_contacts.identifier = @identifier")
function getContactsNotify(identifier)
    local rows = vRP.query("vRP/getContactsNotify",{ identifier = identifier })
    if #rows >= 1 then
        display = rows[1].display
        return display or nil
    else
        return nil
    end
end
...
function addMessage(source,identifier,phone_number,message)
    ...
    local recive = getContactsNotify(sourcePlayer)
    if recive then
        TriggerClientEvent("Notify",otherIdentifier,"importante","Você recebeu uma nova mensagem de <b>"..recive.."</b>")
    else
        TriggerClientEvent("Notify",otherIdentifier,"importante","Você recebeu uma nova mensagem de <b>"..myPhone.."</b>")
    end
end
